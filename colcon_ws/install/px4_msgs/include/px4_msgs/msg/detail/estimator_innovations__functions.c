  ...
  px4_msgs__msg__EstimatorInnovations__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4_msgs__msg__EstimatorInnovations * data =
      (px4_msgs__msg__EstimatorInnovations *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_msgs__msg__EstimatorInnovations__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          px4_msgs__msg__EstimatorInnovations__fini(&data[i]);
        }
        free(data);
        return false;
      }
