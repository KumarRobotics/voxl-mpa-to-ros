...
px4_msgs__msg__FailsafeFlags__are_equal(const px4_msgs__msg__FailsafeFlags * lhs, const px4_msgs__msg__FailsafeFlags * rhs)
{
  ...
  // gps_position_invalid
  if (lhs->gps_position_invalid != rhs->gps_position_invalid) {
    return false;
  }
  // auto_mission_missing
  ...
  px4_msgs__msg__FailsafeFlags * output)
{
  ...
  // gps_position_invalid
  output->gps_position_invalid = input->gps_position_invalid;
  // auto_mission_missing
  ...
  px4_msgs__msg__FailsafeFlags__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4_msgs__msg__FailsafeFlags * data =
      (px4_msgs__msg__FailsafeFlags *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_msgs__msg__FailsafeFlags__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          px4_msgs__msg__FailsafeFlags__fini(&data[i]);
        }
        free(data);
        return false;
      }
